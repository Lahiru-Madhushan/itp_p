...
export default function FemaleWarePage() {
  ...
  // ✅ Fetch only FemaleWare products
  useEffect(() => {
  ...
  // ✅ Add to Cart (always with default size "M")
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-xl text-gray-700 font-medium">Loading Female Ware...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-6 pb-12">
        {products.length === 0 ? (
          <div className="text-center py-20">
            ...
            <h2 className="text-2xl font-bold text-gray-700 mb-2">No FemaleWare Found</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {products.map((product) => {
              ...
              return (
                <div
                  key={product._id}
                  className="group bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 overflow-hidden border-2 border-transparent hover:border-pink-300"
                >
                  {/* Carousel */}
                  <div className="relative bg-gray-50 h-64">
                    ...
                    {product.stockQuantity > 0 && product.stockQuantity <= 5 && (
                      <div className="absolute top-4 left-4 bg-pink-400 text-black px-3 py-1 rounded-full text-sm font-medium">
                        Low Stock
                      </div>
                    )}
                  ...
                  <div className="p-6 space-y-4">
                    <h2 className="text-xl font-bold text-gray-900 mb-2 line-clamp-2 group-hover:text-pink-600 transition-colors">
                      {product.name}
                    </h2>
